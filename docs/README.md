...
```shell
# init local content
docker run --privileged -it -v $PWD:/tmp/docsify --entrypoint docsify docsify/demo init /tmp/docsify/gen --local

# play content
docker run --rm -itp 3000:3000 --name=docsify -v $(pwd)/gen:/docs docsify/demo
```
